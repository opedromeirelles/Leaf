    ...
    public class CompraRepository : baseSqlComandos
    {
        ...
        public Compra MapearCompra(SqlDataReader reader)
        {
            return new Compra
            {
                ...
                DtaBaixa = reader["dta_baixa"] != DBNull.Value ? Convert.ToDateTime(reader["dta_baixa"], new CultureInfo("pt-BR")) : (DateTime?)null,
                DtaCancelamento = reader["dta_cancelamento"] != DBNull.Value ? Convert.ToDateTime(reader["dta_cancelamento"], new CultureInfo("pt-BR")) : (DateTime?)null,
